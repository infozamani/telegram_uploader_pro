...
def preview_product(request):
    ...
    try:
        ...
        caption += (
            "در حال تولید کپشن لوکس توسط هوش مصنوعی...\n\n"
            if data["use_ai_caption"]
            else "محصول باکیفیت و خاص\n\n"
        )
        caption += "━━━━━━━━━━━━\n"
...
def confirm_product(request):
    ...
    try:
        ...
        if Product.objects.filter(product_code=data["product_code"]).exists():
            messages.error(
                request, f"شماره محصول «{data['product_code']}» قبلاً استفاده شده!"
            )
            return redirect("upload_product")
